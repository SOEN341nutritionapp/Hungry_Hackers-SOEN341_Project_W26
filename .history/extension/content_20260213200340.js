...
function extractNameFromRow(row) {
  ...
  const candidates = Array.from(row.querySelectorAll("a,span,p,div"))
    ...
    .map(s => ({
      text: s,
      score: (
        (/\d+\s*(g|ml|kg|l|oz|lb|pack|count|ct)/i.test(s) ? 100 : 0) +
        (/cheese|sauce|banana|orange|bread|milk|yogurt|chicken|beef|pasta|rice/i.test(s) ? 50 : 0) +
        s.length
      )
    }))
