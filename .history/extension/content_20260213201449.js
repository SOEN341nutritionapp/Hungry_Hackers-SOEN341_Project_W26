...
// ---------- SCRAPER (using data attributes) ----------
function scrapeCart() {
  const items = [];

  // Strategy 1: Use data-product-name attributes (most reliable)
  const productCards = document.querySelectorAll('[data-product-name]');
  
  for (const card of productCards) {
    const name = card.getAttribute('data-product-name');
    if (!name || !isGoodName(name)) continue;

    // Try to get quantity from data attributes or stepper
    let qty = 1;
    
    // Check data-cart-entry attribute
    const cartEntry = card.getAttribute('data-cart-entry');
    if (cartEntry) {
      try {
        const parsed = JSON.parse(cartEntry);
        if (parsed && parsed.quantity) qty = Number(parsed.quantity);
      } catch {}
    }

    // Check data-min-qty and data-max-qty for current value
    const minQty = card.getAttribute('data-min-qty');
    const maxQty = card.getAttribute('data-max-qty');
    if (maxQty && Number(maxQty) > 1) {
      // If max > min, current is likely between them
      // Look for actual quantity in nearby elements
      const qtyEl = card.querySelector('[data-quantity], input[type="number"]');
      if (qtyEl) {
        const val = qtyEl.getAttribute('data-quantity') || qtyEl.value;
        if (val) qty = Number(val) || qty;
      }
    }

    // Fallback: look for stepper quantity
    if (qty === 1) {
      const stepperQty = extractQtyFromCard(card);
      if (stepperQty) qty = stepperQty;
    }

    items.push({ name: clean(name), quantity: qty });
  }

  // Strategy 2: Fallback to text scraping if no data attributes found
  if (items.length === 0) {
    console.log(`${TAG} No data-product-name found, falling back to text scraping`);
    return scrapeCartFallback();
  }

  // Dedup by name (keep highest quantity)
  const map = new Map();
  for (const it of items) {
    ...
    if (!map.has(key)) map.set(key, it);
      ...
      map.set(key, { ...it, quantity: Math.max(existing.quantity, it.quantity) });
    }
  }
...
// Fallback scraper using text (old method)
function scrapeCartFallback() {
...
function extractQtyFromCard(card) {
  // Look for quantity steppers or displays within this card
  const stepperContainer = card.querySelector('.stepper, [class*="quantity"]');
  if (!stepperContainer) return null;

  // Look for number between buttons
  const text = clean(stepperContainer.textContent);
...
const BAD_PATTERNS = [
  ...
  /price\s+valid/i,             // "Price valid on Subtotal:"
  /regular\s+price/i,           // "Regular price"
  /subtotal/i,                  // subtotal-related UI
  /valid\s+on/i                 // "valid on..." messages
];
