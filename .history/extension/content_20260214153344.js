...
// --- unit details: reads head__unit-details block like "(190 g avg.)" or "400 g" ---
function extractUnitDetails(productEl) {
  const detailsEl = productEl.querySelector(".head__unit-details");
  if (!detailsEl) return {};

  // Get the raw text
  const raw = detailsEl.textContent.replace(/\s+/g, " ").trim(); // "400 g" or "(190 g avg.)"
  
  // Try to find .unit-factor element first (banana case)
  const unitFactorEl = detailsEl.querySelector(".unit-factor");
  let unitFactor = unitFactorEl ? parseInt(unitFactorEl.textContent.trim(), 10) : null;
  
  // If no .unit-factor element, parse directly from text (cheese case)
  if (!unitFactor) {
    const numberMatch = raw.match(/(\d+)\s*(g|kg|ml|l|lb|oz)/i);
    if (numberMatch) {
      unitFactor = parseInt(numberMatch[1], 10);
    }
  }
